...
const viewOrderManage = async(req,res,next)=>{
    ...
    try {
        ...
        const OrderData = await orderModel.find({}).sort({date:-1}).populate("products.product")
        const OrderCount = await orderModel.find({}).populate("products.product").count()
