...
const viewDashboard = async (req, res, next) => {
    ...
    try {
        ...
        const orderApproved = await orderModel.find({orderStatus:"Approved"}).count()
        const orderPacked = await orderModel.find({orderStatus:"Packed"}).count()
        const orderShipped = await orderModel.find({orderStatus:"Shipped"}).count()
        const orderCancel = await orderModel.find({orderStatus:"Canceled"}).count()
        const orderDeli = await orderModel.find({orderStatus:"Delivered"}).count()
        const orderReturn = await orderModel.find({orderStatus:"Returned"}).count()
        const returnRequest = await orderModel.find({orderStatus:"Return Request"}).count()
        const returnRejected = await orderModel.find({orderStatus:"Return Rejected"}).count()
        const orderRejected = await orderModel.find({orderStatus:"Rejected"}).count()
...
const viewSales = async (req, res, next) => {
    ...
    try {
        ...
        const userCount = await userModel.count();
        const productCount = await productModel.find().count()
        const salesCount = await orderModel.find({orderStatus:"Delivered"}).count()
