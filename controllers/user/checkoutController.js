...
const checkCoupon = async (req, res, next) => {
    ...
    try {
        ...
        const checkCoupon = await couponModel.findOne({ couponCode:req.body.code })
        const couponStatus = await couponModel.findOne({ couponCode:req.body.code,status:"Active"});
        const usedCoupon = await orderModel.findOne({user_Id:userId,"coupon.couponCode":req.body.code});
            ...
            if(couponStatus){
                ...
                if(!usedCoupon){
                    ...
                    if(couponExp>date){
                        ...
                        await couponModel.updateOne({couponCode:req.body.code},{$set:{status:"Expired"}})
...
const submitCheckout = async (req,res,next) => {
    ...
    try {
        ...
        const cartData = await userModel.findOne({_id:id}).populate('cart.product_id')
        ...
        const orderData = await orderModel({
            ...
            if(payment=="COD"){
                ...
                await orderModel.updateOne({_id:OrderId._id},{$set:{paymentMethod:"COD"}})
                ...
                await orderModel.updateOne({_id:OrderId._id},{$set:{paymentStatus:"Paid",paymentMethod:"Wallet"}})
...
const viewCheckoutReview = async (req, res, next) => {
    ...
    try {
        ...
        const orderData = await orderModel.findOne({user_Id:userId}).sort({ date: -1 }).limit(1).populate("products.product");
