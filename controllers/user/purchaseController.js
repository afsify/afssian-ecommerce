...
const viewOrder = async (req, res, next) => {
    ...
    try {
        ...
        const orderDetails = await orderModel
                    ...
                    .sort({date:-1}).populate('products.product')
...
const viewOrderDetail = async (req, res, next) => {
    ...
    try {
        ...
        const orderData = await orderModel
                    .findOne({_id : id }).populate('products.product');
...
const cancelOrder = async(req,res,next)=>{
    ...
    try {
        ...
        if(orderCheck.paymentStatus == 'Paid'){
            ...
            let walletData = await userModel.updateOne({_id:cusId},{$inc:{wallet:userData.Subtotal}})
            ...
            const reOrderStock = await orderModel.findOne({_id:orderId})
                ...
                reOrderStock.products.forEach(async(items,i) => {
                    ...
                    let orderProduct = await productModel.updateOne({_id:items.product},{$inc:{stock:orderQty}})

                });

            const order = await orderModel.updateOne({_id:orderId},{$set:{orderStatus:"Canceled"}})
...
const returnOrder = async(req,res)=>{
    ...
    try {
        ...
        const reOrder = await orderModel.findOneAndUpdate(

            {user_Id:id,_id:orderId},{$set:{orderStatus:"Return Request",returnReason:reason}},{ new: true }
